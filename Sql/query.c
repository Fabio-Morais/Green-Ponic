...
/*MEDIÇOES ATUADORES*/
int enviaMedicoesAtuadores (PGconn *conn,char *atuador, char *roomName, char* estado){
	char query[250];
	char newAtuador[150];
...
int enviaSensor(PGconn *conn, char *nomeSensor, int num_mot){
	char query[250];
	char newSensor[150];
	sprintf(newSensor, "%s%d",nomeSensor, num_mot);
...
int enviaAtuador(PGconn *conn, char *nomeAtuador, char *nomeDivisao, int indiceDivisao){
	char query[250];
	char newAtuador[150];
	sprintf(newAtuador, "%s_%s",nomeAtuador, nomeDivisao);
...
int enviaRegraGeral(PGconn *conn, int linha, char *nomeSensor,int num_mot, char *operador, int valor, char *nomeAtuador, char *nomeDivisao, int indiceDivisao, char *estadoAtuador){
	...
	char newAtuador[150];
	char newSensor[150];
	sprintf(newSensor, "%s%d",nomeSensor, num_mot); /*	TEMP1	*/
	sprintf(newAtuador, "%s_%s",nomeAtuador, nomeDivisao);/*	HEATER_ALFACE	*/
...
int insereAtuadoresDefault(PGconn *conn, Room *room){
	for(int j=0; j<room->countRoom; j++)
		for(int i=0; i< room->roomName[j].countActuador; i++)
			enviaMedicoesAtuadores(conn, room->roomName[j].ActuadorName[i], room->roomName[j].placeName, "off");		
}
