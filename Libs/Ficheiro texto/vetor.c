...
int vetor_insere_Actuador(vetor* vec, char* valor, int pos_room)
{
    ...
    printf("\n\n\n");
    for(int z=0; z<vec->elementos[pos_room].ActuadorCount; z++)
        printf("\n\n\n\t%d---->%s\t count: %d", pos_room,vec->elementos[pos_room].ActuadorName[z], vec->elementos[pos_room].ActuadorCount);
    return pos_room;
}
